...
Jérôme Guélat has compiled a very nice online resource on how to [process spatial data and make maps](https://rpubs.com/jguelat/spatial-data-processing-with-r).

### Date on x-axis
When date is a predictor in a model, we often use the day of year (or julian day, 1-365) as a covariate (unless only a short period is used, often with polynomials, see Chapter \@ref(transformationspolynomials), and if it stretches the entire year, ideally also as circular variable, see Chapter \@ref(transformationscircular); an alternative would be to use month as a fixed factor). On plots, we can plot the day of year, but it is nice and more understandable, if we plot selected real dates, e.g. 1.1, 1.3, 1.5 etc. Here are two options how this can be achieved using R base (there might be easy ggplot-options, too).
...
```{r fig.cap="Displaying (simulated) data with date on the x-axis with custom labelling. Code version 1."}
...
Here is a version with date-format objects that might also be handy (and adaptable in many ways).

```{r fig.cap="Displaying (simulated) data with date on the x-axis with custom labelling. Code version 2.", fig.width=4, fig.height=3.5}
