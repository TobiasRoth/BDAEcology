...
```{r eval=FALSE}
# after installing or updating brm, the got warnings regarding "make not found" and MCMC sampling did not start. The following may help (solution found here: https://discourse.mc-stan.org/t/error-when-running-brm-model/27951) 

# after installing rtools, rstan and brm, run this in the console:
...
From these, we can say that we estimate the slope of the regression line to be $`r round(t.fit["b_x"],2)`$ with 95% compatiblitiy interval $`r round(t.ci["2.5%","b_x"],2)`; `r round(t.ci["297.5%","b_x"],2)`$

To make an effect plot from a model fitted with brm, wen can use the function `conditional_effects`. It makes an effectplot, but we are more flexible when we store the conditionl_effects-object and plot as we please.
