...
```{r scatterplot, echo=F, fig.width=4, fig.height=3, fig.cap="Scatterplot of the length of forearm and the comfort temperature of statistics course participants."}  
par(mar=c(4,4,1,1))
...
```{r correlations, echo=F, fig.width=7, fig.height=4, fig.cap="Covariance (q), Pearson correlation (r), Spearman correlation ($r_s$), and Kendall\'s tau ($\\tau$) for different patterns of x-y-data."}  
cplot <- function(x,y) {
  plot(t.x, t.y, xlab="", ylab="", xaxt="n", yaxt="n", pch=16, xlim=c(0,10),ylim=c(0,10))
  legend("topleft", bty="n", legend=c(bquote(q == .(round(cov(t.x,t.y),3))),
                                      bquote(r == .(round(cor(t.x,t.y),3))),
                                      bquote(r[s] == .(round(cor(t.x,t.y,method="spearman"),3))),
                                      bquote(tau == .(round(cor(t.x,t.y,method="kendall"),3)))),
         cex=1.2)
}
...
plot(x1-mean(x1),x2-mean(x2), asp=1, pch=16, col="blue", xlab=expression(x[1]),
     ylab=expression(x[2]), cex=0.8)
...
```{r ssp, fig.align='center', echo=FALSE, fig.cap='Snowfinches stay above the tree line for winter. They come to feeders.'}
...
```{r nht, fig.cap='Illustration of a one-sample t-test. The blue histogram shows the distribution of the measured Snowfinch weights. The sample mean is indicated as a lightblue vertical line. The black line is the t-distribution that shows how hypothetical sample means are expected to be distributed if the entire population of Snowfinches has a mean weight of 40&nbsp;g (as assumed by the null hypothesis). The orange area shows the area of the t-distribution that lays equal or farther away from 40&nbsp;g than the sample mean. The orange area reflects the p-value.', fig.width=8, fig.height=5, fig.align='center',echo=FALSE}
...
```{r ranhist, fig.cap='Histogram of differences between the means of random assignement of ell values to groups (grey), and the difference between the means of the two observed groups (red).', echo=FALSE}
par(mar=c(5.5, 4.5, 0.5, 0), cex=0.8)
hist(diffH0, main=NA, xlab="Difference among the two groups"); abline(v=coef(mod)[2], lwd=2, col="red", main=NA)
legend("topleft",pch=16, col=c("gray","red"),legend=c(expression(according~to~H[0],"in the real data")), bty="n")
```
