...
Recently, the function 'logst' from Werner Stahel has become available in the package plgraphics, with the [vignette](https://r-forge.r-universe.dev/plgraphics/doc/logst.pdf) explaining the rationale. This function also uses a constant calculated from the quantiles as described above, but applies the transformation only to small values (smaller than the constant). This seems a great option to use in the future!
