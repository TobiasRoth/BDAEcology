...
```{r diagplotsprout, fig.cap="The four standard residual plots obtained by using the plot-function."}
...
```{r overdisp, echo=FALSE, fig.cap="Histogram of a binomial distribution without overdispersion (orange) and one with the same total number of trials and average success probability, but with overdispersion (blue)."}
y <- rbinom(100000, size=20, prob=0.7)
yoverdisp <- rbinom(100000, size=20, prob=rbeta(100000, 5, 2.142857))   # 5/(5+2.142857) = 0.7
hist(yoverdisp, col="blue", ylim=c(0, 20000), xlab="y", cex.lab=1.4, cex.axis=1.2, main="")
hist(y, col="orange", add=TRUE)
hist(yoverdisp, col=NA, border="blue", ylim=c(0, 20000), add=TRUE)
text(0, 20000, "~Binom(0.7, 20)", col="orange", cex=1.4, adj=0)
text(0, 18000, "~Binom(Beta(5, 2.1), 20)", col="blue", cex=1.4, adj=0)
...
For the moment, we use the binomial GLM to analyze the tree sprout data. This model suffers from overdispersion and thus, the uncertainty intervals will be too small. We will provide a more appropriate analyses in a later chapter. 
...
```{r simpois, fig.cap="Simulated data (dots) with a Poisson regression line (solid) and the lower and upper bound of the 95% compatibility interval."}
...
```{r diagpois, fig.cap="Standard residual plots for the Poisson model fitted to simulated data, thus they fit perfectly."}
par(mfrow=c(2,2))
plot(mod)
```
