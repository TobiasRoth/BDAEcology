...
<p class="caption">(\#fig:triplot)Prior distribution, likelihood, and posterior distribution of the mean q. The plot has been drawn using the R function `triplot.normal.knownvariance` provided in the R package blmeco.</p>
</div>
...
<p class="caption">(\#fig:triplottheta)Prior, likelihood, and posterior distribution of the mean $\theta$ using different prior distributions for $\theta$.</p>
</div>
...
##   .. .. ..$ : NULL
##   .. .. ..$ : chr "(Intercept)"
##   ..@ sigma: num [1:5000] 9.78 7.28 7.23 8.22 11.34 ...
```
...
<p class="caption">(\#fig:jointpost)There are 5000 draws from the joint posterior distribution of $\theta$ and $\sigma$. Lower left panel: every dot is one draw from the joint posterior distribution of $\theta$ and $\sigma$. The histograms in the upper and right panels give the marginal posterior distributions of $\theta$ and $\sigma$, respectively.</p>
</div>
