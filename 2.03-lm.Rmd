...
- We use the program Stan via the packages rstanarm and brms to directly draw from the joint posterior distribution

The second option is "fully Bayesian" in that it does not first fit the line with LS. Even though the two approaches (LS and fully Bayesian) are completely different, both yield the same solution (given flat priors for the Bayesian approach), i.e., the line that minimizes the squared residuals.

We now first fit the line using `lm` + `sim`, then using functions from rstanarm and brms. Then, we visualize the results for each approach.
...
Using `lm`, as already mentioned, is not Bayesian. But we can easily go the Bayesian way thanks to the function `sim` from the package arm. It takes a model object fitted with `lm` (or `glm`, `lmer`, `glmer` and a few others) and returns draws from the joint posterior distribution (we can also say that `sim` "simulates" values from the posterior, but we prefer "to draw"). From the joint posterior distribution, we can get the estimated value for each parameter as well as its uncertainty, e.g., the 95% compatibility interval.

We can use the joint posterior distribution (often just called "the posterior") to draw effectplots that show the relationship of y on x, including the uncertainty. An effect plot is essentially a "derived parameter". A derived parameter is any quantity that we calculate using the estimated model parameters. In the case of a simple linear regression, we use the intercept and slope to get fitted values across the range of the x values, including  the uncertainty for each fitted value. We can also calculate other derived parameters from the posterior - whatever makes sense to understand the system we are studying. This is the great magic about the Bayesian approach: whatever derived parameter we want to look at, we simply calculate it for each of the for example 4000 draws from the posterior. Thereby, we propagate the uncertainty in the parameter estimates to our derived parameter (error propagation).

Note that in the case of a simple linear regression, the regression line is defined by the intercept and slope. For the line itself, we don’t need to think of it as a “derived parameter”. But we don’t easily get the uncertainty of this line. When we think of the line as a series of fitted values (across x), we treat each fitted value as a derived parameter, and we get the uncertainty of each fitted value by calculating it using each of the (for example) 4000 draws from the posterior. The spread of these 4000 values represents the uncertainty of the fitted value.
...
The marginal posterior distribution of $\sigma^2$ is independent of specific values of $\boldsymbol{\beta}$. For ﬂat prior distributions, it is an inverse chi-square distribution $p(\sigma^2|\boldsymbol{y,X})=Inv-\chi^2(n-k,\sigma^2)$, where $\sigma^2 = \frac{1}{n-k}(\boldsymbol{y}-\boldsymbol{X,\hat{\beta}})^T(\boldsymbol{y}-\boldsymbol{X,\hat{\beta}})$, and $k$ is the number of parameters. The marginal posterior distribution of $\boldsymbol{\beta}$ can be obtained by integrating the conditional posterior distribution, $p(\boldsymbol{\beta}|\sigma,\boldsymbol{y,X})=normal(\boldsymbol{\hat{\beta}},V_\beta\sigma^2)$, over the distribution of $\sigma^2$ . This results in a uni- or multivariate $t$-distribution.  

Because `sim` draws values $\beta_0^*$ and $\beta_1^*$ always conditional on $\sigma^*$, one draw from the joint posterior distribution contains a triplet of values ($\beta_0^*$, $\beta_1^*$, $\sigma^*$). When we visualize the distribution of the drawn values for one parameter only, ignoring the values for the other, we display the marginal posterior distribution (or, simply, the marginal posterior) of that parameter. For the parameter $\beta_0$, the marginal posterior is a $t$-distribution, even though a normal distribution has been used to draw the values of $\beta_0$. The $t$-distribution is a consequence of using a different $\sigma^2$-value for every draw of $\beta_0$.
</div>
</font>
...
```{r simfirstexample, echo=FALSE, fig.dim = c(8,6.5), fig.cap="Joint posterior distribution (scatterplots) and marginal posterior distributions (histograms) of the model parameters. The six scatterplots use different axes to show the three-dimensional cloud of 1000 draws from the joint posterior distribution of the three parameters."}
histofun <- function(x) {
...
The posterior distribution describes what values are most likely to correspond to the true parameter value, given the data and the model (assumptions). The scatter in the posterior distribution expresses the uncertainty of the parameter estimates. It shows what we know about the model parameter after having looked at the data, given the model is realistic.

The posterior distribution has as many dimensions as there are parameters to estimate. To say something about one parameter, we look at the marginal posterior distribution of that parameter. This is simply the joint posterior distribution projected onto the axis of the parameter (histograms in Figure \@ref(fig:simfirstexample)). The 2.5% and 97.5% quantiles of the marginal posterior distribution can be used as a 95% compatibility interval of the model parameter.
...
To really understand what our model tells us, we make an effect plot. But first, we will discuss the importance of checking the model assumptions. Then, we will provide alternative ways of getting draws from the posterior using rstanarm and brms. Finally, we will talk about making the effect plot to show the relationship between x and y.


#### Checking model assumptions

Conclusions drawn from a model depend on the model assumptions. When model assumptions are violated, estimates are usually biased and we likely draw inappropriate conclusions. We devote Chapter \@ref(residualanalysis) to assessing model assumptions, given its importance. Checking model assumptions is equally important, independent of whether you use `sim` (on a model object from `lm`, `glm`, `lmer`, `glmer`) or the following two approaches to get draws from the joint posterior distribution using packages rstanarm and brms.


#### When we use rstanarm and brms{#brm}

Above, we have used `sim` to get draws from the posterior. This works fine and is fast, but we are limited in the model types we can use (for example, there is no zero-inflated model, autocorrelation cannot be included, etc.), and there are no downstream functions once we have the draws. If we cannot do the task with `sim`, we generally do it with the functions from rstanarm, which offer more flexibility. If it does not work with these, we use the package brms.
...
- Function `sim` from package arm: This function takes a model object fitted with `lm`, `glm` or a function from the lme4 package or polr, and returns draws from the joint posterior distribution. Computationally, this approach is fastest and safest (there are fewer warnings, and fewer issues with convergence).
- Functions of the package rstanarm: These functions use Stan under the hood. This approach is fully Bayesian, e.g., you may provide informative priors. It fits some more models than the first approach (e.g., negative binomial models), it is faster than brms, and it has less convergence problems. There are great downstream options (e.g., posterior predictive model checking, cross-validation), but fewer than with `brm`.
- Function `brm` from package brms: As rstanarm, it uses Stan. This approach takes more time for fitting but offers many additional options such as treating autocorrelation (and much more!). It also offers additional downstream options (e.g. the function `conditional_effects` to make effect plots).
- Stan: This is a powerful MCMC sampler, but coding the model in Stan itself is more difficult.
- Bugs (WinBugs, OpenBugs) and jags: These are MCMC samplers intensively used in ecology, for example for fitting (also very complex) models with latent variables such as survival models, integrated population models, etc. Model coding may be learned using books like @Kery.2010 and @Kery.2012.
- nimble: like Bugs, but maybe faster for some models, and with more flexibility to for example create custom MCMC samplers.

For the models treated in this book, we extensively use the first three approaches, generally choosing the simplest possible method (i.e., `sim` if possible; otherwise rstanarm; otherwiese `brm`) for computation speed reasons. Using rstanarm or `brm` may be chosen for models feasible also with `sim` for the great downstream options (simple generation of effectplots, especially helpful when such are needed for random slope models, posterior predictive model checking, or cross-validation).
...
For the MCMC sampling done under the hood, we need some more arguments (all have defaults): `warmup` specifies the number of iterations during which we allow the algorithm to adapt to our specific model and to converge to the posterior distribution (similar to the burn-in period when using, e.g., Gibbs sampling). These iterations are discarded automatically. `iter` specifies the total number of iterations (including those discarded). `chains` specifies the number of chains. `init` specifies the starting values of the iterations. By default (i.e., `inits=NULL`, or by setting `inits="random"`), the initial values are randomly chosen, which is recommended because then different initial values are chosen for each chain which helps to identify non-convergence. However, random initial values sometimes cause the Markov chains to behave badly. Then you can either use the maximum likelihood estimates of the parameters as starting values, or simply ask the algorithm to start with zeros. `thin` specifies the thinning of the chain, i.e., whether all iterations should be kept (thin=1), or for example that only every 4th should be kept (thin=4). `cores` specifies the number of computer cores used for the algorithm. `seed` specifies the random seed, allowing for replication of results.


#### Fitting a linear regression using rstanarm

Functions from the package rstanarm are often almost as fast as using `sim`. The function we use for the simple linear regression is `stan_glm`. This is a bit tricky, because there is also a function called `stan_lm`, which does the linear regression somewhat differently (with regularizing priors) and requests additional arguments. Table \@ref(tab:ModelsPackages) provides an overview of the most common data distributions and the corresponding functions from packages stats, lme4, rstanarm and brms.
...
Remember that the regression line found in this fully Bayesian approach is the same as the line found by the least squares approach using `lm` (this analogy holds as long as we use flat priors)!


#### Fitting a linear regression using brm

Next, we fit the above linear regression using `brm` and default settings for priors and MCMC sampling. The default family is gaussian, i.e., we also don't need to specify the family argument for a normal linear model (such as a normal linear regression).

```{r fit_lm_brm, eval=FALSE}
mod.brm <- brm(y~x, data=dat)
```

For more complex models, fitting time can be quite long. This is the case if you for example have a large data set or if you want to account for spatial autocorrelation by fitting a Gaussian process using the `+ gp()` notation in the model formula. In that case, it is advisable to use more cores, e.g., `cores=4`.

Before Stan can draw from the posterior distribution (a process often referred to by "MCMC sampling"), the model has to be compiled. This step can take quite some time. If you want to compile without MCMC sampling, e.g., to save the compiled model for later re-use without waiting for compilation again, you can (as suggested by Paul Bürkner [here](https://discourse.mc-stan.org/t/separating-compilation-sampling-with-brms-on-cluster/8905/2)) set `chains=0`, optionally save the compiled model, and use `update` with the argument `recompile=FALSE` to do the actual MCMC sampling. At this stage, you can adjust the MCMC sampling settings such as the number of cores, the number of iterations, thinning, etc., or set a random seed.

```{r}
# compile the model without MCMC sampling, save the compiled model for 
# later use, get MCMC samples from the compiled model using update with
# recompile=F.
...
# refresh=0 prevents printing the MCMC progress; this makes sense here 
# in the book, but in your application you may prefer to see the progress

```

The MCMC sampling is not guaranteed to converge, and/or the returned samples may be autocorrelated to a degree that your effective number of draws from the joint posterior is nowhere near 4000 (which is the default number of draws returned by rstanarm-functions and `brm`). For simple models like a linear regression, there will not generally be a problem, but for more complex models, issues about the validity of the returned draws appear quite regularly. In Chapter \@ref(stan) we show how to check MCMC convergence, and we discuss the warnings that appear quite regularly when using `brm`.
...
```{r figlmer2, fig.cap = "Regression with 95% compatibility interval of the posterior distribution of the fitted values.", fig.asp=0.45}
...
The poly-transformation generates rather small numbers (in our case, values are between `r min(t.poly) %>% round(3)` and `r max(t.poly) %>% round(3)`), hence their coefficient estimates are often conspicuously large (e.g. in the 100s). If you attach importance to having comparable coefficient estimates (as discussed in Chapter on transformations \@ref(ztransformation)), you can z-transform the orthogonal polynomials. But this cannot be done in the model formula, i.e. don't use `scale(poly(Water,2))` - it fits, but it is a different model that will not fit to your data. You can use `scale(Water.l)`, or you prepare everything in the data frame:
