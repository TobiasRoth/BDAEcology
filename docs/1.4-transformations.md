...
![(\#fig:unnamed-chunk-5)log-transformation of an original variable (leftmost two panels), for a variable with larger values (above row of panels) and one with smaller values (lower row). Adding 1 or the Box-Cox transformation don't seem to help much, adding a constant according to Stahel performs best in these cases.](1.4-transformations_files/figure-docx/unnamed-chunk-5-1.png)
...
![(\#fig:unnamed-chunk-7)Observed relationship between the size of the study area and the number of species found there (circles) with a linear-quadratic regression line (red) to describe this relationship. While this line is a reasonable model for the data at hand, it is nonsense to extrapolate the line beyond the available data (blue line).](1.4-transformations_files/figure-docx/unnamed-chunk-7-1.png)
...
![(\#fig:unnamed-chunk-10)Relationship between wind direction and migration activity (circles; virtual data). We fit a linear regression through the data using two circular wind variables (north and east components).](1.4-transformations_files/figure-docx/unnamed-chunk-10-1.png)
