...
`stan_lmer` is "fully Bayesian" (while `lmer` is not Bayesian, and `lmer` + `sim` returns Bayesian draws from the posterior not in a fully Bayesian way, i.e. it assumes flat priors and this cannot be changed). Therefore, it is possible to use custom priors if needed. 

By default, rstanarm creates 
