...
Reverend Thomas Bayes (1701 or 1702 - 1761) developed the Bayes theorem. Based on this theorem, he described how to obtain the probability of a hypothesis given an observation, that is, data. However, he was so worried whether it would be acceptable to apply his theory to real-world examples, that he did not dare to publish it. His methods were only published posthumously [@Bayes.1763]. Without the help of computers, Bayes’ methods were only applicable to simple problems.  
...
```{r gaussdistpd, fig.cap="Density function of a Gaussian (=normal) distribution with mean 1 kg and standard deviation 0.2 kg. The vertical lines indicate the three observations in the data. The curve has been produced using the function dnorm."}
...
The likelihood (`r round(l, 1)`, as calculated in the code above) does not tell us much. However, another web page says that prairie dogs weigh on average 1.2 kg with a standard deviation of 0.4 kg. The likelihood for this alternative model is:
...
Thus, the probability of observing the three weights (0.8, 1.2, and 1.1 kg) is four times larger given the first model compared to the second model.  
...
```{r ml, fig.cap="Likelihood function for y=0.8, 1.2, and 1.1, and the data model y~normal(mu, sigma). The cross indicates the highest likelihood value."}
...
The combination of mean = `r round(MLest$par[1],2)` and standard deviation = `r round(MLest$par[2],2)` is associated with the highest likelihood value (the “mountain top” in Figure \@ref(fig:ml)). In
...
We prepare a matrix "pyi" to be filled by the posterior densities for every observation $y_i$ and for each of the simulated sets of model parameters.
...
@Link.2010 provide a very understandable chapter on likelihood.

Also, @Burnham.2002 give a thorough introduction to likelihood and how it is used in the multimodel inference framework. An introduction to likelihood for mathematicians, but also quite helpful for biologists, is given by @Dekking.2005.
