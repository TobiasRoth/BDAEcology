...
![(\#fig:gaussdistpd)Density function of a Gaussian (=normal) distribution with mean 1 kg and standard deviation 0.2 kg. The vertical lines indicate the three observations in the data. The curve has been produced using the function dnorm.](2.01-bayesian_paradigm_files/figure-docx/gaussdistpd-1.png)
...
![(\#fig:ml)Likelihood function for y=0.8, 1.2, 1.1 and the data model y~normal(mu, sigma). The cross indicates the highest likelihood value.](2.01-bayesian_paradigm_files/figure-docx/ml-1.png)
