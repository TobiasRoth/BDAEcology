...
To answer the question about how strongly $y$ is related to $x$ taking into account statistical uncertainty we look at the joint posterior distribution of $\boldsymbol{\beta}$ (vector of length 2, i.e. with $\beta_{0}$ and $\beta_{1}$) and $\sigma^2$ , the residual variance. The function `sim` calculates the joint posterior distribution and returns simulated values from this distribution. 
...
```{r figlmer3, cache = TRUE, fig.cap = "Regression line with 95% compatibility interval (dotted lines) and the 95% interval of the simulated predictive distribution (broken lines). Note that we increased the number of simulations to 50,000 to produce smooth lines.", fig.asp=0.45}
