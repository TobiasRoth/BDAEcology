...
According to our experience, `rstanarm` can be installed from CRAN as any R-package without problems. For installing `brms`, we recommend making sure that the versions of R, RTools and RStudio are compatible with the `brms` version that will be installed (see also \@ref(brm)). For [installing rstan](https://github.com/stan-dev/rstan/wiki/RStan-Getting-Started), it is advised to follow closely the system-specific instructions. 
...
```{r checkconvergencemodelbrm, fig.cap="Traceplot of the Markov chains. After convergence, the Markov chains should sample from the same stationary distribution. Indications of non-convergence would be if the chains diverge or vary around different means.", fig.height=2.5}
