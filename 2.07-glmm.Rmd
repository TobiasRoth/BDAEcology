...
A slight overdispersion (dispersion parameter >1) seems to be present, but nothing to worry about.
...
# fk: der Default ist flache priors für die Effektsizes, die in logistischen Modellen sehr ungeschickt sind. Fürs Intercept braucht es t mit df=3 und sigma 2.5, was ich nicht so schlecht finde.  Ich fände es schön, wenn wir die Modelle jeweils mit spezifizierten priors zeigen.

# - Modell so korrekt? Und ist hier der begriff mixed-effects eigentlich obsolet, oder kann man den auch verwenden?
# fk: -> ja sicher, weshalb meinst du, ist das obsolet? Wir haben ja immer noch modellierte (random effects) und unmodellierte (fixed effects) Parameter. Der Begriff mixed-models ist auch für frequentistsiche Modelle ungeschickt gewählt. 
