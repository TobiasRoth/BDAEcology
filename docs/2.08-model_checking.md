...
![(\#fig:histpp)Histograms of 8 out of 4000 replicated data sets and of the observed data (dat$bp). The arguments breaks and ylim have been used in the function hist to produce the same scale of the x- and y-axis in all plots. This makes comparison among the plots easier.](2.08-model_checking_files/figure-docx/histpp-1.png)
...
The observed data contain 47% zero values, which is well within the 98%-range of what the model predicted (3 - 95%). the Bayesian p-value is 0.6.
...
![(\#fig:agepp)Histogram of the average age of the 10% wildflower fields with the highest breeding densities in the replicated data sets. The orange line indicates the average age for the 10% fields with the highest observed whithethroat densities.](2.08-model_checking_files/figure-docx/agepp-1.png)
