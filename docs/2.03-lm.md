...
![(\#fig:figlm)Linear regression. Black dots = observations, blue solid line = regression line, orange dotted lines = residuals. The fitted values lie where the orange dotted lines touch the blue regression line.](2.03-lm_files/figure-docx/figlm-1.png)
...
![(\#fig:simfirstexample)Joint distribution (scatterplots) and marginal (histograms) posterior distributions of the model parameters. The six scatterplots show, using different axes, the three-dimensional cloud of 1000 draws from the joint posterior distribution of the three parameters.](2.03-lm_files/figure-docx/simfirstexample-1.png)
...
![(\#fig:figlmer1)1000 regression lines corresponding to 1000 draws from the joint posterior distribution for the intercept and slope parameters to visualize the uncertainty of the estimated regression line.](2.03-lm_files/figure-docx/figlmer1-1.png)
...
![(\#fig:figlmer2)Regression with 95% compatibility interval of the posterior distribution of the fitted values.](2.03-lm_files/figure-docx/figlmer2-1.png)
...
![(\#fig:figlmer3)Regression line with 95% compatibility interval (dotted lines) and the 95% interval of the posterior predictive distribution (broken lines). Note that we increased the number of draws to produce smooth lines.](2.03-lm_files/figure-docx/figlmer3-1.png)
...
![(\#fig:figanova)Weights (g) of the 30 individuals in each group. The dark horizontal line is the median, the box contains 50% of the observations (i.e., the interquartile range), the whiskers mark the range of all observations that are less than 1.5 times the interquartile range away from the edge of the box.](2.03-lm_files/figure-docx/figanova-1.png)
...
![(\#fig:figanovares)Distribution of the simulated values from the posterior distributions of the group means (left); group means with 95% compatibility intervals obtained from the simulated distributions (right).](2.03-lm_files/figure-docx/figanovares-1.png)
...
![(\#fig:fgwingpa)Wing length measurements on 19 museumm skins of coal tits per age class and sex. Fitted values are from the additive model (black triangles) and from the model including an interaction (black dots). Vertical bars = 95% compatibility intervals.](2.03-lm_files/figure-docx/fgwingpa-1.png)
