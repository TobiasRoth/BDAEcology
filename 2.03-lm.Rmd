...
While the first part of Equation \@ref(eq:lm) describes the regression line, the second part describes how the data points, i.e. the observations, are distributed around the regression line (Figure \@ref(fig:illlm)). In other words: the observation $y_i$ stems from a normal distribution with mean $\mu_i$ and variance $\sigma^2$. The mean of the normal distribution, $\mu_i$ , equals the sum of the intercept ($b_0$ ) and the product of the slope ($b_1$) and the continuous predictor value, $x_i$. $y_i$ is also often called the fitted value, or the estimate or estimated value.
...
We prefer the notation in Equation \@ref(eq:lm) because, in this formula, the stochastic part (second row) is nicely separated from the deterministic part (first row) of the model, whereas, in the second notation \@ref(eq:lmalternativ) the ﬁrst row contains both stochastic and deterministic parts. Also, for a generalized linear model, only Equation \@ref(eq:lm) can be formulated.

For illustration, we here simulate a data set and below ﬁt a linear regression to them. The advantage of simulating data is that the following analyses can be reproduced without having to read data into R. Further, for simulating data, we need to translate the algebraic model formula into R language which helps us understand the model structure.
...
where $\boldsymbol{ I}$ is the $n \times n$ identity matrix (it transforms the variance parameter to a  $n \times n$ matrix with its diagonal elements equal $\sigma^2$ ; $n$ is the sample size). The multiplication by $\boldsymbol{ I}$ is necessary because we use vector notation, $\boldsymbol{y}$ instead of $y_{i}$ . $\boldsymbol{y}$  is the vector of all observations (i.e. the y-values), whereas $y_{i}$ is a single observation, $i$. When using vector notation, we write the linear predictor of the model, $\beta_0 + \beta_1 x_i$ , as a multiplication of the model matrix
...
- We use the function `brm` from the package brms to directly draw from the joint posterior distribution

The second option is "fully Bayesian" in that it does not first fit the line with LS. Even though the two approaches (LS vs. fully Bayesian) are completely different, both yield the same solution (given flat priors for the Bayesian approach), i.e. the line that minimizes the squarred residuals. We now first fit the line using `lm`, then using `brm`, and then we draw inference for each of the two approaches.
...
#### Introduction to the brm function{#brm}

Below we fit the same model, i.e. the same linear regression, using the function `brm` from the R package brms [@Burkner.2017]. This function allows fitting Bayesian generalized (non-)linear multivariate multilevel models using Stan [@Betancourt.2013b] for full Bayesian inference. We shortly introduce the fitting algorithm used by Stan, Hamiltonian Monte Carlo, in chapter \@ref(stan).

When using the function `brm`, there is no need write the model in Stan-language. You can use model coding familiar from functions such as `lm`, `glm`, `lmer` and `glmer`, and `brm` translates it to Stan code, sends it to be fitted in Stan (via the rstan package) and returns the draws from the joint posterior distribution. A wide range of data distributions (Gauss, Poisson, negative Binomial and many more) and link functions are supported, and the function offers many things more: autocorrelation, truncation, smoothing (GAM), easy posterior predictive model checking, etc.

For `brm` to work, we need the package rstan, and for that to work, we need to install Rtools (for Windows) or Xcode (for Mac). Search the web for instructions on these points. An alternative may be to use the package [cmdstanr](https://mc-stan.org/cmdstanr/articles/cmdstanr.html#comparison-with-rstan), but we have only limited experience so far. Also check out the resources on the [brms github repository](https://github.com/paul-buerkner/brms).

Prior distributions are an integral part of a Bayesian model. We can see what default prior distributions `brm` is using by applying the `get_prior` function to the model formula. For our model, we see that the default prior for the intercept and effect size of x is a flat prior. This is a distribution with a constant density for any value between minus and plus infinity (what constant value does not matter). Because this is not a proper probability distribution, it is also called an improper distribution. The intercept gets a t-distribution with 3 degrees of freedom and its  mean and standard deviation depend on the data. For the standard deviation, it uses the same t-distribution as for the intercept but with mean 0. However, because variance parameters such as standard deviations only can take on positive numbers, it will use only the positive half of the t-distribution. In the output from `get_prior` this is indicated by the zero in the column labelled "lb" for "lower bound".
...
For the MCMC sampling done under the hood, we need some more arguments: `warmup` specifies the number of iterations during which we allow the algorithm to adapt to our specific model and to converge to the posterior distribution. These iterations should be discarded (similar to the burn-in period when using, e.g., Gibbs sampling); `iter` specifies the total number of iterations (including those discarded); `chains` specifies the number of chains; `init` specifies the starting values of the iterations. By default (`inits=NULL`; or by setting `inits="random"`) the initial values are randomly chosen, which is recommended because then different initial values are chosen for each chain which helps to identify non-convergence. However, sometimes random initial values cause the Markov chains to behave badly. Then you can either use the maximum likelihood estimates of the parameters as starting values, or simply ask the algorithm to start with zeros. `thin` specifies the thinning of the chain, i.e., whether all iterations should be kept (thin=1) or for example every 4th only (thin=4); `cores` specifies the number of computer cores used for the algorithm; `seed` specifies the random seed, allowing for replication of results.

In the following chapter we use `brm` to fit the linear regression. We discuss a number of additional points regarding brm, e.g. what warnings may appear (with more complex models) and how to deal with them.
...
mod.brm <- update(mod.brm.compiled, recompile = FALSE, seed=123, refresh=0)
    ...
    # book, but in your application you may prefer to see the progressm
...
- Warnings about too high R-hat and/or too low ESS must be addressed in all cases. These warnings tell us that the model or parts of it have not converged and anything concluded from these draws may be complete nonsense. Often, the problem can be resolved by increasing the number of iterations, e.g. `iter=20000`. Since the warnings indicate that your MCMC draws are autocorrelated (thereby reducing the effective sample size ESS of your number of draws), you may thin the chains to prevent generating a lot of redundant numbers, e.g. `thin=5`. R-hat is a measure of independence of the draws and, hence, contains essentially the same information as the ESS: an R-hat of 1 means no autocorrelation and ESS will be close to the draws there are (i.e. 4000 by default), a larger R-hat means some degree of autocorrelation (it's good to have all R-hat <1.1). Generally, we get one R-hat value and two ESS values per estimated parameter.The two ESS values are "bulk ESS" and "tail ESS". The first tells us about how many effectively independent draws we have in the middle of the marginal posterior distribution of the parameter. This should be >200 or better >2000. The tail ESS is about the effective number of draws in the two tails of the posterior, which is relevant for the quality of the compatibility interval estimate as this is defined as the 2.5% to 97.5% quantile of the posterior, i.e. it depends on the quality of the draws in the tails of the posterior. Again, it should probably be >200 or better >2000. Note that, as said, R-hat and ESS are given per parameter. If all values are fine but one or few parameters of little interest (e.g. a random effect level you are not much interested in) show R-hat of >1.1 and rather small ESS, and increasing the iterations does not help, you may still be content with the model, but you would not draw great inferences from the random effect levels that converged poorly.
...
Finally, if you want to dig deeper into Stan and potentially customize the code to your needs when they cannot be met by brm, you can ask for the Stan code generated by brm using `make_stancode`, which can be a great starting point for your specific model.

```{r, echo=FALSE, eval=FALSE}
make_stancode(y~x,data=dat)
```


### Drawing conclusions

To answer the question about how strongly $y$ is related to $x$ taking into account statistical uncertainty we look at the joint posterior distribution of $\boldsymbol{\beta}$ (vector of length 2, i.e. with $\beta_{0}$ and $\beta_{1}$) and $\sigma^2$ , the residual variance. To get the joint posterior distribution, as stated above, we can use `sim` or `brm`, or some other options not covered here (see the box above). From the joint posterior distribution, we can get, for each parameter, the estimated value (the "point estimate") and its uncertainty (we use the 95% compatibility interval).

We can use the posterior to draw effect plots that show the relationship of y on x including the uncertainty. An effect plot is sequentially a "derived parameter" from the parameter estimates, because we use the estimates, here intercept and slope, and draw the regression line, including its uncertainty. We can also calculate other derived parameters from the posterior - whatever makes sense to understand the system under study. This is the great magic about the Bayesian approach: whatever derived parameter we want to look at, we simply calculate it for each of the e.g. 4000 draws from the (joint) posterior (distribution), and we propagated the uncertainty in the parameter estimates to our derived parameter (error propagation).

#### Drawing conclusions using sim

The function `sim` applied to a lm-object (or glm, lmer, glmer and a few others) returns draws from the joint posterior distribution (or "simulated values" from the posterior distribution, but we prefer "draws" in this context).
...
The 2.5% and 97.5% quantiles of the marginal posterior distributions can be used as 95% compatibility intervals of the model parameters. The function `coef` extracts the drawn values for the beta coefﬁcients, returning a matrix with *nsim* rows and the number of columns corresponding to the number of parameters. In our example, the ﬁrst column contains the draws from the posterior distribution of the intercept and the second column contains draws from the posterior distribution of the slope. The "2" in the second argument of the `apply` function indicates that the `quantile` function is applied columnwise.
...
We often want to show the effect of $x$ on $y$ graphically, with information about the uncertainty of the parameter estimates included in the graph. To make such effect plots, we use the draws from the posterior distribution of the model parameters. From the deterministic part of the model, we know the regression line $\mu = \beta_0 + \beta_1 x_i$. The draws from the joint posterior distribution of $\beta_0$ and $\beta_1$ are 1000 pairs of intercepts and slopes, which actually describe 1000 different regression lines. We can plot these regression lines in an x-y plot (scatter plot) to show the uncertainty in the regression line estimation (Fig. \@ref(fig:figlmer1)). Note, that in this case it is not advisable to use `ggplot` because we draw many lines in one plot, which makes `ggplot` rather slow.
...
```{r}
apply(bsim,2,mean)
apply(bsim,2,quantile,probs=c(0.025,0.975))
```

From these, we can say that we estimate the slope of the regression line to be $$



We are now very close to the bsim we generated above where we used `lm` + `sim` to get the many regression lines plotted in \@ref(fig:figlmer1). Above, we used the bsim in combination with the model matrix to get values for each regression line in \@ref(fig:figlmer1) across the range of the x-values (and we stored all that in fitmat). For that, we first had to create a newdat with the 100 (or so) values of x from the minimum to the maximum. All that (newdat, model matrix, fitmat) can be done with one function that takes a brm-object: `conditional_effects`. It makes an effectplot, but we are more flexible when we store the conditionl_effects-object and plot as we please.

```{r}
t.e.x <- conditional_effects(mod.brm)
